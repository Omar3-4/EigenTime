...
export function BlockGeneratorModal({
  ...
  const updateWeight = (id: string, weight: number) => {
    setSelected(
      selected.map((s) => (s.subjectId === id ? { ...s, weight: Math.max(1, weight) } : s)),
    );
  };
  ...
  const handleGenerate = () => {
    ...
    const remainder = totalMinutes - allocatedSum;

    if (Math.abs(remainder) > 0 && Math.abs(remainder) < 15) {
      // Find largest block
      allocations.sort((a, b) => b.finalMinutes - a.finalMinutes);
      if (allocations[0])
        if (allocations[0]) if (allocations[0]) allocations[0].finalMinutes += remainder;
    ...
    const playlist: PlaylistConfig[] = allocations
      .filter((a) => a.finalMinutes > 0)
      .map((a) => {
            ...
            <div className="grid grid-cols-2 gap-2">
              <button
                ...
                className={cn(
                  ...
                  sliceStyle === "25_5"
                    ? "bg-[var(--focus-soft)] text-[var(--focus-foreground)] border border-[var(--focus)]"
